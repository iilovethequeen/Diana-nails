...
// Demo: random booked slots (replace with real data from backend if needed)
const now = new Date();
for (let i = 0; i < 10; i++) {
...
function updateSummary() {
  ...
  if (selectedDate && selectedSlot) {
    ...
    s.innerHTML = `
      <span style="color:var(--gold);font-family:'Cinzel',serif;font-size:.65rem;letter-spacing:.25em">YOUR SELECTION</span>
      <br/><br/>
      📅 <strong style="color:var(--cream)">${label}</strong>
      &nbsp;&nbsp;
      🕐 <strong style="color:var(--cream)">${selectedSlot}</strong>
    `;
  } else {
...
function submitAppointment() {
  ...
  .then(res => {
    if (res.ok) {
      // Mark slot as booked locally so it shows as unavailable
